...
import com.mzone.mymoviesapi.adapter.RecyclerTVAdapter;
...
public class FragmentTV extends Fragment {
    ...
    private RecyclerTVAdapter adapter;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        final RecyclerView recyclerView = view.findViewById(R.id.rvTV);
        recyclerView.setLayoutManager(new LinearLayoutManager(getActivity()));
        recyclerView.setHasFixedSize(true);

        adapter = new RecyclerTVAdapter(getActivity());
    ...
    private void loadDiscoveryTV() {
        mainViewModel.getTVResponseLiveData().observe(this, tvResponse -> {
            if (tvResponse != null) {
                pbTV.setVisibility(View.GONE);
                List<TVResult> TV = tvResponse.getResults();
                adapter.setTV(TV);
            } else {
                pbTV.setVisibility(View.GONE);
    ...
    private void loadSearchResult() {
        mainViewModel.getTVSearchResponseLiveData().observe(this, tvResponse -> {
            if (tvResponse != null) {
                pbTV.setVisibility(View.GONE);
                List<TVResult> TV = tvResponse.getResults();
                if (TV.size() == 0) {
